...
def main():
    ...
    recent_games["FGA_5game_avg"] = (
        recent_games["FGA"].shift(1).rolling(window=5, min_periods=1).mean()
    )
    ...
    last_10_debug = all_games_sorted[["GAME_DATE", "MATCHUP", "PTS"]].head(10).copy()
    ...
    last_10 = all_games_sorted.head(10).copy()
    ...
    st.markdown(
        f"<h4 style='color: #F2F4FF;'>Predicted Points: {prediction_from_avg:.2f}</h4>",
        unsafe_allow_html=True,
    )
